...
const Card: NextPage<{
  ...
  useEffect(() => {
    user.savedMovies && setSaved(!!user.savedMovies.filter(movie => {
    ...
    .mutate({
      mutation: gql`
        mutation addSavedMovie($movieName: String!, $movieId: Int!, $movieImage: String!) {
    addSavedMovie(movieName: $movieName, movieID: $movieId, movieImage: $movieImage) {
      id
    name
    email
    image
  ...
  return (
    <div className="py-5 flex flex-col font-poppins text-text-dark rounded-2xl opacity-95 hover:opacity-100 relative">
      ...
      <div className={`absolute right-10 top-10 p-5 ${saved&&"bg-secondary-dark"} hover:bg-primary-dark text-4xl rounded-full cursor-pointer`} onClick={() => saved ?handleRemoveSavedMovie() : handleSavingMovie()}>
        <span className="">
          <BsBookmark />
        </span>
      </div>
